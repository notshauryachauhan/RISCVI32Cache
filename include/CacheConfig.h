...
// Custom compile-time base-2 logarithm for power-of-two integers
constexpr uint32_t const_log2(uint32_t n) {
    return (n <= 1) ? 0 : 1 + const_log2(n >> 1);
}
...
constexpr uint32_t NUM_INDEX_BITS = const_log2(NUM_SETS);
constexpr uint32_t NUM_OFFSET_BITS = const_log2(BLOCK_SIZE);
